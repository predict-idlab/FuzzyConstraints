...
from sampling.standard.batching import BatchLoader
...
class Trainer(GenericTrainer):
    ...
    def _start(self, batch, model, prec=False):
        ...
        if prec:
            if not os.path.exists(os.path.join("pickles", "pickle_" + self.dataset_name, str(self.neg_ratio) + "_batches_standard.pkl")):
                batches = self.precompute(batch,
                                          self.num_epochs, num_batches_per_epoch, single=False)
                dump_to_pickle(batches, self.dataset_name, str(self.neg_ratio) + "_batches_standard.pkl")
            else:
                batches = load_from_pickle(self.dataset_name, str(self.neg_ratio) + "_batches_standard.pkl")
    ...
    def _eval(self, model, batch, is_valid=False):
        ...
        def test_prediction(x_batch, y_batch, head_or_tail='head'):
            ...
            for i in range(len(x_batch)):
                ...
                print("rank:", _filter, "relation:", rel)
                ...
                print("scores:", micro_mr[rel], micro_mrr[rel], micro_hits1[rel], micro_hits5[rel], micro_hits10[rel])
            ...
            print("batch totals:", micro_num,
                  batch_micro_mr / micro_num,
        ...
        if not self.attributive:
            enhanced = "regular"
        wri = open("results/standard/" + self.mode + "_" + self.dataset_name + "_f" + str(self.fold) + "_"
                   + enhanced + "_" + strict + "_r" + str(self.neg_ratio) + "_reference", 'w')
