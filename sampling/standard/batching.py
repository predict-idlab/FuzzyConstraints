    ...
    def create_fuzzy_sets(self, name):
        ...
        def membership(predicate, entity, domain=True):
            ...
            if domain:
                ...
                # if strictly valid, we assume a baseline compatibility score of 0.5
                if self.strict and self.validate_entity(predicate, entity, domain=True) == 0:
                    con_score_sp, con_score_ge = 0.5 + 0.5 * con_score_sp, 0.5 + 0.5 * con_score_ge
                ...
                if type(entity) is Literal:
                    ...
                    return 0.5 * (0.2 * con_score_ge + 0.8 * con_score_sp) + 0.5 * lit_score
                ...
                # if strictly valid, we assume a baseline compatibility score of 0.5
                if self.strict and self.validate_entity(predicate, entity, domain=False) == 0:
                    con_score_sp, con_score_ge = 0.5 + 0.5 * con_score_sp, 0.5 + 0.5 * con_score_ge
        ...
        for p in tqdm(all_properties):
            for e in list(self.entity2id.keys()):
                ...
                self.domains[(p, e)] = 0.2 * domain_car_scores[(p, e)] + 0.8 * membership(p, e, domain=True)
                self.ranges[(p, e)] = 0.2 * range_car_scores[(p, e)] + 0.8 * membership(p, e, domain=False)
