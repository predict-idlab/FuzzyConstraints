...
from sampling.strict.batching import BatchLoader
...
class Trainer(GenericTrainer):
    ...
    def _start(self, batch, model, prec=False):
        ...
        if prec:
            if not os.path.exists(os.path.join("pickles", "pickle_" + self.dataset_name, str(self.neg_ratio) + "_batches_strict.pkl")):
                batches = self.precompute(batch,
                                          self.num_epochs, num_batches_per_epoch, single=False)
                dump_to_pickle(batches, self.dataset_name, str(self.neg_ratio) + "_batches_strict.pkl")
            else:
                batches = load_from_pickle(self.dataset_name, str(self.neg_ratio) + "_batches_strict.pkl")
    ...
    def _eval(self, model, batch, is_valid=False):
        ...
        if not self.attributive:
            enhanced = "regular"
        wri = open("results/strict/" + self.mode + "_" + self.dataset_name + "_f" + str(self.fold) + "_"
