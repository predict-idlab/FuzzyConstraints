...
from sampling.clustering.batching import BatchLoader
...
class Trainer(GenericTrainer):
    ...
    def train_and_eval(self, mode="TransE", train_data=None, test_data=None, valid_data=None):
        ...
        batch = BatchLoader(model, multimodal, self.dataset_name, self.head_tail_selector,
    ...
    def _start(self, batch, model, prec=False):
        ...
        def update_batch(epoch):
            # update clusters
            if epoch % 5 == 0:
                batch.set_clusters(dist=False)

        if prec:
            if not os.path.exists(os.path.join("pickles", "pickle_" + self.dataset_name, str(self.neg_ratio) + "_batches_clustering.pkl")):
                batches = self.precompute(batch,
                                          self.num_epochs, num_batches_per_epoch,
                                          update=update_batch, single=False)
                dump_to_pickle(batches, self.dataset_name, str(self.neg_ratio) + "_batches_clustering.pkl")
            else:
                batches = load_from_pickle(self.dataset_name, str(self.neg_ratio) + "_batches_clustering.pkl")
    ...
    def _eval(self, model, batch, is_valid=False):
        ...
        def predict(ph, pt, pr):
            with torch.no_grad():
                new_batch = self._convert_htr(ph, pt, pr)
        ...
        if not self.attributive:
            enhanced = "regular"
        wri = open("results/clustering/" + self.mode + "_" + self.dataset_name + "_f" + str(self.fold) + "_"
