...
class BatchLoader(GenericBatchLoader):
    def __init__(self, model, multimodal, dataset_name, headTailSelector,
                 types_per_entity=None, entities_per_type=None,
                 ontology=None, train_store=None, fold=0,
                 batch_size=128, neg_ratio=1.0, strict=True):
            ...
            for i in range(last_idx):
                for j in range(self.neg_ratio):
                    ...
                    if (np.random.randint(np.iinfo(np.int32).max) % 1000) > pr:
                        # valid = True
                        valid = self._validate_perturbation(rdm_words, cur_idx,
    ...
    def _validate_perturbation(self, rdm_words, cur_idx, prev_word, dist=False, head=True):
        replacement = self.indexes_words[rdm_words[cur_idx]]
        original = self.indexes_words[prev_word]
        if dist and replacement in self.clusters:
            pr = random()
            if head:
                o_pr = self.clusters[original][0]
                r_pr = self.clusters[replacement][0]
            else:
                o_pr = self.clusters[original][1]
                r_pr = self.clusters[replacement][1]
            ratio = len(o_pr.intersection(r_pr)) / max(len(o_pr), len(r_pr))
            return ratio >= pr
        return replacement in self.clusters and self.clusters[original] == self.clusters[replacement]

    def set_clusters(self, dist=False):
        if not dist:
            with torch.no_grad():
                embeddings = self.model.ent_embeddings(torch.LongTensor(list(range(self.model.ent_tot)))).numpy()
            clustering = KMeans(self.model.ent_tot // 25)
            clustering.fit(embeddings)
            labels = clustering.labels_

            clusters = dict()
            for idx, label in enumerate(labels):
                clusters[self.id2entity[idx]] = label
        else:
            clusters = dict()
